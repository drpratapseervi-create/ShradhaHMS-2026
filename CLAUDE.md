...
**Before starting any server on port 8000, check for AND KILL every process already on that port — not just the current listener.** Windows can end up with more than one process bound/attempting to bind to port 8000 at once (e.g. a stray `waitress` instance left running from an earlier "production-style" check, alongside a fresh `runserver`) — whichever process actually answers a given request is effectively random, so a page can look "sometimes fixed, sometimes not" across reloads even though the code is correct and only one of the servers is stale. `Get-NetTCPConnection -LocalPort 8000 -State Listen | Stop-Process` only catches the process currently holding the listen socket, not other processes still running and competing for it — instead enumerate by command line first (`Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where-Object { $_.CommandLine -like "*port=8000*" -or $_.CommandLine -like "*runserver*" }`) and kill every match before launching a new server. Confirmed 2026-09-06: this exact scenario (two orphaned `waitress` processes plus a fresh `runserver`, all on port 8000) was the real cause of a CSS fix that appeared to apply inconsistently.
